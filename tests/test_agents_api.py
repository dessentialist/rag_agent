...
def test_agents_crud_flow(test_app_client):
    ...
    resp = test_app_client.post('/api/agents', data=json.dumps(payload), content_type='application/json')
    assert resp.status_code in (201, 400)  # 201 when unique, 400 if duplicate
    if resp.status_code == 201:
        agent_id = resp.get_json()['id']
        # Update
        resp = test_app_client.put(f'/api/agents/{agent_id}', data=json.dumps({"description": "Updated"}), content_type='application/json')
        assert resp.status_code == 200
        # Delete
